...
class AdvanceDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some advanced methods
    ...
    @classmethod
    def from_advance_conf(self, advance_dot_conf):
        from advance_config import AdvanceConfig
        config_text = AdvanceConfig.slurp_config_file(advance_dot_conf)
        creds = AdvanceConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = advancelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = advancelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = advancelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_advanced_proto_version_with_sentinel_ping)
