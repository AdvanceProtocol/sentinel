...
def is_valid_advance_address(address, network='mainnet'):
    ...
    advance_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Advance address
        return False

    if (address_version != advance_version):
        return False
...
# shims 'til we can fix the advanced side
def SHIM_serialise_for_advanced(sentinel_hex):
    from models import ADVANCED_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = ADVANCED_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in advanced
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # advanced expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    advanced_hex = serialise(obj)
    return advanced_hex


# shims 'til we can fix the advanced side
def SHIM_deserialise_from_advanced(advanced_hex):
    from models import ADVANCED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(advanced_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('advance.conf')
