...
def has_advance_conf():
    import config
    import io

    valid_advance_conf = False

    # ensure advance_conf exists & readable
    #
    # if not, print a message stating that Advance Protocol must be installed and
    # configured, including JSONRPC access in advance.conf
    try:
        f = io.open(config.advance_conf)
        valid_advance_conf = True
    except IOError as e:
        print(e)

    return valid_advance_conf
...
def main():
    ...
    if not has_advance_conf():
        print("AdvanceProtocol must be installed and configured, including JSONRPC access in advance.conf")
        sys.exit(1)
