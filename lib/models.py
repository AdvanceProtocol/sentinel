...
class GovernanceObject(BaseModel):
    ...
    # sync advanced gobject list with our local relational DB backend
    @classmethod
    def sync(self, advanced):
        golist = advanced.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_advanced(advanced, item)
    ...
    @classmethod
    def import_gobject_from_advanced(self, advanced, rec):
        import decimal
        import advancelib
        import inflection
        ...
        # shim/advanced conversion
        object_hex = advancelib.SHIM_deserialise_from_advanced(object_hex)
        objects = advancelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from advanced...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from advanced, with every run
        ...
        # get/create, then sync payment amounts, etc. from advanced - Advanced is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(advanced)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from advanced! %s" % e)
            govobj.vote_delete(advanced)
            return (govobj, None)
    ...
    def vote_delete(self, advanced):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(advanced, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, advanced, signal, outcome):
        import advancelib
        ...
        output = advanced.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = advancelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(advanced, signal)

    def sync_network_vote(self, advanced, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = advanced.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ADVANCED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 advance addr, non-multisig
            if not advancelib.is_valid_advance_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Advance address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (advancelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/AdvanceDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import advancelib
        obj_data = advancelib.SHIM_serialise_for_advanced(self.serialise())
    ...
    def prepare(self, advanced):
        try:
            object_hash = advanced.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ADVANCED_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not advancelib.is_valid_advance_address(addr, config.network):
    ...
    def hash(self):
        import advancelib
        return advancelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = ADVANCED_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, advanced):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - advanced.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, advanced):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - advanced.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, advanced):
        now = int(time.time())
        return (self.created_at < (now - advanced.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, advanced):
        if self.is_expired(advanced):
            return False

        return True

    def is_deletable(self, advanced):
        if self.is_expired(advanced):
            return True
